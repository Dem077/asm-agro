...
use App\Http\Requests\BulkAssetCheckinRequest;
...
class BulkAssetsController extends Controller
{
    ...
    public function storeCheckout(AssetCheckoutRequest $request) : RedirectResponse | ModelNotFoundException
    {
        ...
        try {
            ...
            $formUserId = $this->resolveHandoverFormUserId(
                $request->filled('form_user_id') ? (int) $request->input('form_user_id') : null,
                $target
            );
            ...
            DB::transaction(function () use ($request, $target, $formUserId, $admin, $checkout_at, $expected_checkin, &$errors, $assets, &$formResult) {
                foreach ($assets as $asset) {
                ...
                if (empty($errors) && $formUserId) {
                    $formResult = $this->createHandoverForm($formUserId, $assets);
                }
            });

            $this->downloadform = $request->input('download_form');

            if (empty($errors) && $this->downloadform && $formResult) {
                return redirect()->route('handover-form.download', $formResult['form_id'])->with([
                    'success' => trans_choice('admin/hardware/message.multi-checkout.success', count($asset_ids)),
                ]);
            }

            if (! $errors) {
                return redirect()->to('hardware')->with('success', trans_choice('admin/hardware/message.multi-checkout.success', count($asset_ids)));
            }
    ...
    /**
     * Show bulk check-in page.
     */
    public function showCheckin(): View
    {
        $this->authorize('checkin', Asset::class);

        $alreadyCheckedIn = collect();

        if (old('selected_assets') && is_array(old('selected_assets'))) {
            $assets = Asset::findMany(old('selected_assets'));

            [$assignable, $alreadyCheckedIn] = $assets->partition(function (Asset $asset) {
                return (bool) $asset->assigned_to;
            });
        ...
        return view('hardware/bulk-checkin', [
            ...
            'removed_assets' => $alreadyCheckedIn,
        ]);
    }

    /**
     * Process bulk check-in for selected deployed assets.
     */
    public function storeCheckin(BulkAssetCheckinRequest $request): RedirectResponse
    {
        $this->authorize('checkin', Asset::class);

        $assetIds = array_filter($request->input('selected_assets', []));

        if (empty($assetIds)) {
            return redirect()->route('hardware.bulkcheckin.show')
                ->withInput()
                ->with('error', trans('admin/hardware/message.multi-checkin.no_assets_selected'));
        }

        $assets = Asset::with('model', 'assetform')->findOrFail($assetIds);

        $notCheckedOut = $assets->filter(fn (Asset $asset) => ! $asset->assigned_to);
        if ($notCheckedOut->isNotEmpty()) {
            return redirect()->route('hardware.bulkcheckin.show')
                ->withInput()
                ->with('error', trans('admin/hardware/message.multi-checkin.not_checked_out'));
        }

        $errors = [];
        $checkedInAssets = collect();
        $admin = auth()->user();
        ...
        DB::transaction(function () use ($request, $assets, $admin, $checkinAt, &$errors, &$checkedInAssets) {
            foreach ($assets as $asset) {
                $this->authorize('checkin', $asset);

                if (! $asset->model) {
                    $errors[$asset->id][] = trans('admin/hardware/general.model_invalid_fix');
                ...
                if (is_null($target)) {
                    $errors[$asset->id][] = trans('admin/hardware/message.checkin.already_checked_in');
                ...
                if ($request->filled('checkin_at') && $request->input('checkin_at') != date('Y-m-d')) {
                    $originalValues['action_date'] = $checkinAt;
                }
                ...
                )->get()->each(function ($acceptance) {
                    $acceptance->delete();
                });

                if ($asset->save()) {
                    event(new CheckoutableCheckedIn($asset, $target, $admin, $request->input('note'), $checkinAt, $originalValues));
                    $checkedInAssets->push($asset);
                } else {
                    $errors = array_merge_recursive($errors, $asset->getErrors()->toArray());
                }
            }
        ...
        if (! empty($errors)) {
            return redirect()->route('hardware.bulkcheckin.show')
                ->withInput()
                ->with('error', trans_choice('admin/hardware/message.multi-checkin.error', count($assetIds)))
                ->withErrors($errors);
        }

        $formResult = null;
        if ($checkedInAssets->isNotEmpty()) {
            $formResult = $this->processBulkAssetReturns($checkedInAssets);
        }

        if ($request->input('download_form') && $formResult) {
            return redirect()->route('return-form.download', $formResult['form_id'])->with([
                'success' => trans_choice('admin/hardware/message.multi-checkin.success', count($assetIds)),
            ]);
        }

        return redirect()->to('hardware')->with(
            'success',
            trans_choice('admin/hardware/message.multi-checkin.success', count($assetIds))
        );
    }
